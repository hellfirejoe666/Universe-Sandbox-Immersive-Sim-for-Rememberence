...
function savePost(content, parentId, author) {
    let posts = JSON.parse(localStorage.getItem('rememberencePosts') || '[]');
    let counters = JSON.parse(localStorage.getItem('rememberencePostCounters') || '{}');
    counters.lastId = (counters.lastId || 0) + 1;
    const newPost = {
        id: counters.lastId.toString(),
        content,
        parentId,
        author,
        timestamp: Date.now(),
    ...
    posts.push(newPost);
    console.log('Saving post:', newPost, 'Total posts:', posts.length);
    localStorage.setItem('rememberencePosts', JSON.stringify(posts));
    localStorage.setItem('rememberencePostCounters', JSON.stringify(counters));
    return newPost.id;
}
