...
function saveState() {
    const spirit = {
        name: document.getElementById('spirit-name').value,
    ...
    if (!spirit.name || !spirit.animal || !spirit.star) {
        console.error(`Invalid spirit data for saving: ${JSON.stringify(spirit)}`);
        alert('Cannot save spirit: Name, Animal, and Star Sign are required.');
        return;
    }
    const spirits = JSON.parse(localStorage.getItem('rememberenceSpirits') || '[]');
    const existingIndex = spirits.findIndex(s => s.name.toLowerCase() === spirit.name.toLowerCase());
    const bios = calculateBiorhythms(spirit.animal, spirit.star);
    spirit.thoughts = generateThoughts(bios); // Generate thoughts for new or updated spirit
    spirit.loyaltyMap = spirit.loyaltyMap || { Player: 0 }; // Initialize loyaltyMap
    if (existingIndex !== -1) {
        spirits[existingIndex] = { ...spirits[existingIndex], ...spirit };
        console.log(`Updated spirit ${spirit.name} in localStorage`);
    } else {
        spirits.push(spirit);
        console.log(`Saved new spirit ${spirit.name} to localStorage`);
    }
    localStorage.setItem('rememberenceSpirits', JSON.stringify(spirits));
    alert(`Spirit ${spirit.name} saved! Its thoughts echo in the cosmos.`);
}
...
function loadState() {
    ...
    if (selectedName) {
        const state = spirits.find(s => s.name.toLowerCase() === selectedName.toLowerCase());
        if (state) {
            document.getElementById('spirit-name').value = state.name;
            document.getElementById('description').value = state.description || '';
            document.getElementById('animal-sign').value = state.animal || 'Rat';
            document.getElementById('star-sign').value = state.star || 'Aries';
            document.getElementById('species').value = state.species || '';
            document.getElementById('species2').value = state.species2 || '';
            populateSpeciesActiveSkills(state.species, state.species2);
            document.getElementById('species-active-skill').value = state.speciesActive || '';
            document.getElementById('type').value = state.type || '';
            document.getElementById('type2').value = state.type2 || '';
            populateTypeActiveSkills(state.type, state.type2);
            document.getElementById('type-active-skill').value = state.typeActive || '';
            document.getElementById('melee-skill').value = state.meleeSkill || '';
            document.getElementById('ranged-skill').value = state.rangedSkill || '';
            document.getElementById('magic-skill').value = state.magicSkill || '';
            document.getElementById('step-skill').value = state.stepSkill || '';
            document.getElementById('special-skill').value = state.specialSkill || '';
            document.getElementById('trance-skill').value = state.tranceSkill || '';
            document.getElementById('level').value = state.level || 1;
            // Load or regenerate thoughts
            const bios = calculateBiorhythms(state.animal, state.star);
            state.thoughts = state.thoughts || generateThoughts(bios); // Use saved thoughts or regenerate
            localStorage.setItem('rememberenceSpirits', JSON.stringify(spirits)); // Save updated thoughts
            console.log(`Loaded spirit ${state.name} with thoughts: ${JSON.stringify(state.thoughts)}`);
            fillSheet();
            alert('Spirit loaded! Its essence shines anew.');
        } else {
            console.warn(`No spirit found with name: ${selectedName}`);
...
function deleteState() {
    ...
    if (selectedName) {
        const filteredSpirits = spirits.filter(s => s.name.toLowerCase() !== selectedName.toLowerCase());
        if (filteredSpirits.length === spirits.length) {
            alert('No spirit found with that name.');
        } else {
            localStorage.setItem('rememberenceSpirits', JSON.stringify(filteredSpirits));
